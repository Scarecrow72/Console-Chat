...
static const uint one_block_size_bytes {64}; // количество байт в блоке
static const uint one_block_size_uints {16}; // количество 4байтовых в блоке
static const uint block_expend_size_uints {80}; // количество 4байтовых в дополненном блоке
...
{    
    ...
    std::memcpy(newMessage, message, msize_bytes);

    // первый бит ставим '1', остальные обнуляем
    newMessage[msize_bytes] = 0x80;    
    std::memset(newMessage + msize_bytes + 1, 0, needAdditionalBytes - 1);
